...
class Commands extends Command implements PluginOwned{
    ...
    public function __construct(Main $plugin){
		parent::__construct("particle", "Particle main command", "/pa disable/enable | set <id> | list", ["pa"]);
    ...
    public function execute(CommandSender $p, string $label, array $args) : bool{
        ...
        if(!($p instanceof Player)){
            $p->sendMessage(TF::RED."[PlayerParticle] > You can use this command only in-game!");
            return true;
        }
        ...
        switch(strtolower($args[0])){
          case "disable":
            ...
            $p->sendMessage(TF::GREEN."[PlayerParticle] > Particle disabled");
            break;
          case "enable":
            $this->plugin->deco = new Config($this->plugin->getDataFolder()."players/". strtolower($p->getName()) . "/player.yaml", Config::YAML);
            $this->plugin->deco->get("Particle");
            ...
            $this->plugin->deco->save();
            $p->sendMessage(TF::GREEN."[PlayerParticle] > Particle enabled");
            break;
          case "set":
          case "list":
            if(($this->plugin->getServer()->isOp($p->getName()) === true)){
              ...
              if(in_array($args[1], $this->plugin->types)){
                ...
                $this->plugin->deco->save();
                $p->sendMessage(TF::GREEN."[PlayerParticle] > Set particles with ID: ".$args[1]);
              }else{
                $p->sendMessage(TF::RED."[PlayerParticle] > Particles not found, use /pa list to see all available particles");
              }
            }else{
              $p->sendMessage(TF::RED."[PlayerParticle] > You need OP permissions to use that!");
            }
        }
        return true;
      }
